function figs = plotShearBonds(S, outDir)
%PLOTSHEARBONDS Bond-breaking figures Q1-Q2 from phononShearSuite results.
...
%  Only strains before the first omega^2 < 0 are drawn. Works for any crystal.
    ...
    for q = 1:3
        ...
        for k = order'
            ...
            if M.active(k, s)
                ...
                plot([P(1) Qp(1)], [P(2) Qp(2)], '-', 'Color', [cm(ci, :) 0.75], 'LineWidth', 1.2);
            else
                plot([P(1) Qp(1)], [P(2) Qp(2)], '--', 'Color', 'k', 'LineWidth', 2);
                mid = 0.5 * (P + Qp);  plot(mid(1), mid(2), 'kx', 'MarkerSize', 8, 'LineWidth', 1.8);
            end
        end
        plot(cur(:, 1), cur(:, 3), 'o', 'MarkerFaceColor', [0.2 0.2 0.2], 'MarkerEdgeColor', 'w', 'MarkerSize', 6);
        colormap(gca, turbo); clim(clim0); axis equal; box on; xlabel('x'); ylabel('z');
        nBr = sum(~M.active(:, s));
        title(sprintf('\\gamma = %.3f   (%d broken bonds, %.1f%%)', g(s), nBr, 100 * nBr / nB));
        set(gca, 'FontSize', fs, 'LineWidth', 1.1);
        if q == 3, cb = colorbar; cb.Label.String = 'bond stretch (d-d_0)/d_0    [black x = broken]'; end
    end
    ...
    sgtitle(ttl, 'FontSize', fs + 1, 'FontWeight', 'bold');
    finish(figs.Q1, outDir, 'Q1_bond_map');

    figs.Q2 = figure('Color', 'w', 'Position', [80 80 1150 470]);
    subplot(1, 2, 1); hold on;
    broken = ~M.active;
    plot(g(1:nV), 100 * mean(broken(:, 1:nV), 1), 'k-', 'LineWidth', 2.6, 'DisplayName', 'all bonds');
    for sh = 1:numel(shellVal)
        sel = shellId == sh;
        plot(g(1:nV), 100 * mean(broken(sel, 1:nV), 1), '-', 'LineWidth', lw, 'Color', shellCol(sh, :), ...
            'DisplayName', sprintf('d_0 = %.2f (%d bonds)', shellVal(sh), nnz(sel)));
    end
    xlabel('Shear strain \gamma'); ylabel('Broken bonds (%)'); title('When: bond-breaking progression by bond length');
    lg = legend('Location', 'northwest'); lg.FontSize = fs - 3; set(gca, 'FontSize', fs, 'LineWidth', 1.1); grid on; box on;
    ...
    for sh = 1:numel(shellVal)
        ...
        scatter(geo(sel), 100 * M.stretch(sel, s), 36, shellCol(sh, :), 'filled', 'MarkerFaceAlpha', 0.7);
    end
    brk = ~M.active(:, s);
    scatter(geo(brk), 100 * M.stretch(brk, s), 90, 'k', 'x', 'LineWidth', 1.8);
    xx = linspace(min(geo), max(geo), 20);
    plot(xx, 100 * g(s) * xx, 'k--', 'LineWidth', 1.4);
    if strcmp(M.opts.model, 'morse')
        yline(100 * log(2) / M.opts.alphaMorse, ':', 'critical stretch', 'Color', [0.64 0.08 0.18], 'LineWidth', 1.6);
    end
    xlabel('Geometric factor  n_x n_z  of the bond direction'); ylabel(sprintf('Bond stretch at \\gamma = %.3f (%%)', g(s)));
    title('Why: stretch \approx \gamma n_x n_z  (dashed, 1st order); x = broken');
    set(gca, 'FontSize', fs, 'LineWidth', 1.1); grid on; box on;
    finish(figs.Q2, outDir, 'Q2_break_statistics');
end
