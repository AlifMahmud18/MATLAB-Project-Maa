...
%GETELEMENTDETAILS Maps atomic masses to CPK-style element colors and hover names.
%   Covers all 118 elements (H through Og). Matching is tolerant of small
%   deviations in the input mass (e.g. isotope averaging, rounding, or
%   measurement noise) by finding the nearest table entry and accepting
%   it if it's within a combined absolute/relative tolerance.

    Natoms = length(masses);
    colors = zeros(Natoms, 3);
    elementNames = cell(Natoms, 1);

    % Element lookup table: [Mass (amu), RGB Color, Symbol, Name]
    elements = {
    ...
    % Fallback distinct color palette for non-standard or arbitrary mass values
    ...
    for i = 1:Natoms
        ...
        % Find the closest entry in the periodic table by absolute mass difference.
        diffs = abs(tableMasses - m);
        [minDiff, idx] = min(diffs);
        refMass = tableMasses(idx);

        % Tolerate small deviations: accept the nearest match if it's within
        % either a fixed absolute tolerance or a small relative tolerance
        % (scaled to the reference element's mass), whichever is larger.
        absTol = 0.3;
        relTol = 0.01; % 1% of the element's atomic mass
