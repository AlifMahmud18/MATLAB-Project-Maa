...
function newPos = applySymOp(opStr, pos)
    % Evaluates a symmetry operation string like 'x+1/2, -y, z+1/2'
    x = pos(1); y = pos(2); z = pos(3); %#ok<NASGU>
    
    % Clean operation string
    opStr = strrep(opStr, '''', '');
    opStr = strrep(opStr, '"', '');
    ...
    % Evaluate fractional arithmetic (e.g. 1/2) using vector eval
    newX = eval(parts{1});
    newY = eval(parts{2});
    newZ = eval(parts{3});
...
function isDup = isDuplicateAtom(existingAtoms, newPos, tol)
    ...
    % Use bsxfun instead of a bare "existingAtoms - newPos" so this works
    % on older MATLAB releases (pre-R2016b) that lack implicit expansion
    % (auto-broadcasting) for element-wise operators - without it, an
    % [N x 3] minus a [1 x 3] throws "Matrix dimensions must agree".
...
function mass = getAtomicMass(symbol)
    persistent massMap
    if isempty(massMap)
        % Mass dictionary in amu (g/mol).
        % NOTE: dictionary() (R2022b+) and double-quoted string arrays
        % (R2016b+) are not available in MATLAB R2016a, so this uses
        % containers.Map with single-quoted char keys instead - it has
        % worked the same way since R2008b, so it runs unchanged on
        % both R2016a and modern MATLAB.
